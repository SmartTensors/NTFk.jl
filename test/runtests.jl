...
@Base.Test.testset "NTFk TensorDecompositions Tucker analysis" begin
	srand(1)
	csize = (2, 3, 4)
	tsize = (5, 10, 15)
...
@Base.Test.testset "NTFk TensorDecompositions CP analysis" begin
	...
	@show vecnorm(T_orig .- T_est)
	@Base.Test.test isapprox(vecnorm(T_orig .- T_est), 0.047453937969484744, atol=1e-3)
end

@Base.Test.testset "NTFk Tensorly Tucker analysis" begin
	srand(1)
	csize = (2, 3, 4)
	tsize = (5, 10, 15)
	tucker_orig = NTFk.rand_tucker(csize, tsize, factors_nonneg=true, core_nonneg=true)
