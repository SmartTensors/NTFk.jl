...
function computestats(X, Xe, volumeindex=1:size(Xe,1), wellindex=1:size(Xe,3), timeindex=:, c=""; plot=true, quiet=true, wellnames=nothing, xaxis=1:size(Xe,2))
	...
	plot && NTFk.plot2d(X, Xe; quiet=quiet, figuredir="results-12-18", keyword=lowercase(replace(replace(c, " ", "_"), "/", "_")), titletext=c, wellnames=wellnames, dimname="Well", xaxis=xaxis, xmin=Dates.Date(2015,12,15), xmax=Dates.Date(2017,6,10), linewidth=1.5Gadfly.pt)
end
