...
@Test.testset "NTFk TensorDecompositions2 Tucker analysis" begin
	Random.seed!(1)
	csize = (2, 3, 4)
	tsize = (5, 10, 15)
	...
	T_est = TensorDecompositions2.compose(tucker_est[ibest])
	@Test.test csize == csize_est
	display(norm(T_orig .- T_est))
	@Test.test isapprox(norm(T_orig .- T_est), 1.0104725439065432, atol=1e-3)
end

@Test.testset "NTFk TensorDecompositions2 CP analysis" begin
	Random.seed!(1)
	csize = (2, 3, 4)
	tsize = (5, 10, 15)
	tucker_orig = NTFk.rand_tucker(csize, tsize, factors_nonneg=true, core_nonneg=true)
	...
	T_est = TensorDecompositions2.compose(cp_est[ibest])
	display(norm(T_orig .- T_est))
	@Test.test isapprox(norm(T_orig .- T_est), 0.047453937969484744, atol=1e-3)
end

@Test.testset "NTFk TensorDecompositions2 Tucker sparse analysis" begin
	Random.seed!(1)
	csize = (2, 3, 4)
	tsize = (5, 10, 15)
	tucker_orig = NTFk.rand_tucker(csize, tsize, factors_nonneg=true, core_nonneg=true)
	...
	tucker_est, csize_est, ibest = NTFk.analysis(T_orig, [csize], 1; seed=1, eigmethod=[false,false,false], progressbar=false, tol=1e-4, maxiter=100, lambda=0.)
	T_est = TensorDecompositions2.compose(tucker_est[ibest])
	display(norm(T_orig[.!nans] .- T_est[.!nans]))
	@Test.test csize == csize_est
	@Test.test isapprox(norm(T_orig[.!nans] .- T_est[.!nans]), 1.0332311547429105, atol=1e-2)
end
