...
for i in 1:nruns
	...
	@time tucker_spnn = TensorDecompositions.spnntucker(T, sizes[i], tol=1e-15, ini_decomp=:hosvd, core_nonneg=true, max_iter=1000, verbose=true, lambdas=fill(0.1, length(sizes[i]) + 1)))
	T_est = TensorDecompositions.compose(tucker_spnn)
	residues[i] = TensorDecompositions.rel_residue(tucker_spnn)
	for j = 1:ndimensons
		correlations_factors[i,j] = minimum(map(k->cor(tucker_orig.factors[j][:,k], tucker_spnn.factors[j][:,k]), 1:min(sizes[i][j],csize[j])))
	end
