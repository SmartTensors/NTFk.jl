...
trank = 2
tsize = (10, 20, 5)
cp_orig = rand_candecomp(trank, tsize, lambdas_nonneg=true, factors_nonneg=true)
...
for i in 1:nruns
	...
	@time cpe = TensorDecompositions.candecomp(T, tranks[i], factors_initial_guess, compute_error=true, method=:ALS)
	T_est = TensorDecompositions.compose(factors)
	residues[i] = TensorDecompositions.rel_residue(factors)
	for j = 1:ndimensons
		correlations_factors[i,j] = minimum(map(k->cor(cp_orig.factors[j][:,k], cpe.factors[j][:,k]), 1:min(size(cpe.factors[j], 2), size(cp_orig.factors[j], 2))))
	end
