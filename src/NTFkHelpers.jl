...
function remap(v::AbstractVector, vi::Union{AbstractVector,AbstractUnitRange}, ve::Union{AbstractVector,AbstractUnitRange}; nonneg::Bool=true, sp=[Interpolations.BSpline(Interpolations.Quadratic(Interpolations.Line(Interpolations.OnGrid())))], ep=[Interpolations.Line(Interpolations.OnGrid())])
	lv = length(v)
	li = length(vi)
	@assert lv == li
